...
- **规则选片** —— 不调用 LLM。按标题匹配、季集覆盖、画质阶梯打分后生成校验过的决策对象，再按 candidate id 转存；不会按搜索结果原始顺序盲转。确定性解析对齐 MoviePilot MetaInfo（WEB 平台标签、HDR/IMAX 等效果、编码、中文季集/动漫绝对集号、制作组、识别词替换/集偏移、tmdbid 绑定、目录名+文件名联合解析）。活动页会标明「规则选片」或「智能选片」。
