...
- **agent 选片** —— agent 读真实搜索结果,按画质偏好、**HDR 阶梯**（DV > HDR10+ > HDR10 > SDR）、**中文字幕**需求、去重来挑,转存后再回读验证。画质/HDR 词不进搜索关键词。
- **追踪 + 定时补缺** —— 季级状态机;定时巡检默认只回来处理仍有缺集的剧（不会全库升级画质;要巡检也升级须在设置里显式打开）。详见 [画质升级与 DV/HDR](docs/quality-upgrade-and-hdr.md)。
